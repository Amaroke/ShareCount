...
void Accueil::fenetre_creer_un_compte()
{
    ...
    if (inscription->result() == QDialog::Accepted) {
        ...
        Utilisateur nouvel_utilisateur = Utilisateur(adresse_mail, mot_de_passe, nom_utilisateur);
        bool mdp_correct;
...
void Accueil::fenetre_se_connecter(const std::string& adresse_mail, const std::string& mot_de_passe, const bool utilisateur_existe)
{
    ...
    if (connexion->result() == QDialog::Accepted) {
        ...
        if (utilisateur_existe_vraiment && mdp_correct) {
            ...
            FenetreUtilisateur* fenetre_utilisateur = new FenetreUtilisateur(nullptr, gestionnaire_dialogue);
            fenetre_utilisateur->setWindowTitle("Fenêtre utilisateur");
            fenetre_utilisateur->show();
            this->close();
        } else {
            if (utilisateur_existe_vraiment && !mdp_correct) {
